...
public class ThirdlyAdapter extends BaseAdapter {
    private final Context context;
    private List<CzpInfo> list;
    ...
    public ThirdlyAdapter(Context context, List<CzpInfo> list) {
        this.context = context;
        this.list = list;
    }
    public void refreshData(List<CzpInfo> list){
        this.list = null;
        this.list = list;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView != null) {
            ...
            view = View.inflate(context, R.layout.thirdly_item, null);
        ...
        CzpInfo data = list.get(position);

        Spanned temp = Html.fromHtml("<font color=#666666>"
                + "操作内容：" + "</font>");

        String context = data.getCzpContent();
        if (TextUtils.isEmpty(key)) {
            holder.tv_wxjl1.setText(data.getCzpTime() == null ? "" : data.getCzpTime());
            holder.tv_wxjl2.setText(data.getCzpPeople() == null ? "" : data.getCzpPeople());
            ...
            if (!TextUtils.isEmpty(data.getCzpPeople()))
                holder.tv_wxjl2.setText(setKeyWordColor(data.getCzpPeople(), key));
            else
